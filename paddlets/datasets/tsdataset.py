...
class UEAloader(paddle.io.Dataset):
    """
    ...
    def load_all(self, root_path, file_list=None, flag=None):
        ...
        if len(input_paths) == 0:
            raise Exception("No .ts files found using pattern: '{}'".format(
                pattern))
    ...
    def load_single(self, filepath):
        ...
        labels_df = pd.DataFrame(
            labels.cat.codes, dtype=np.
            int8)  # int8-32 gives an error when using nn.CrossEntropyLoss
        ...
        if np.sum(
                vert_diffs
        ) > 0:  # if any column (dimension) has varying length across samples
        ...
        df = pd.concat(
            (pd.DataFrame(
                {col: df.loc[row, col]
                 for col in df.columns}).reset_index(
                     drop=True).set_index(pd.Series(lengths[row, 0] * [row]))
