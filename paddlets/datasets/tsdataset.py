...
def collate_func(data, max_len=None):
    ...
    def padding_mask(lengths, max_len=None):
        ...
        return paddle.arange(
            start=0, end=max_len).astype(dtype=lengths.dtype).tile(
                repeat_times=[batch_size, 1]).less_than(y=lengths.unsqueeze(
                    axis=1))
    ...
    if max_len is None:
        max_len = max(lengths)
    X = paddle.zeros(shape=[batch_size, max_len, features[0].shape[-1]])

    for i in range(batch_size):
        end = min(lengths[i], max_len)
        X[(i), :end, :] = features[i][:end, :]
    targets = paddle.stack(x=labels, axis=0)
    # print('lengths, features[0].shape', lengths, features[0].shape)
    # [1751, 1751, 1751, 1751, 1751, 1751, 1751, 1751, 1751, 1751, 1751, 1751, 1751, 1751, 1751, 1751] [1751, 3] 
...
class UEADataset(paddle.io.Dataset):
    """
    ...
    def __init__(self, dataset, limit_size=None, renorm=None):
        ...
        mean = self.concate_array.mean()
        std = self.concate_array.std()
        for dfi in self.feature_df:
            dfi._target._data = (dfi._target._data - mean) / (
    ...
    def load_all(self, dataset):
        ...
        self.max_seq_len = len(df[0]._target)
        # df = pd.concat((pd.DataFrame({col: df.loc[row, col] for col in df.
        #     columns}).reset_index(drop=True).set_index(pd.Series(lengths[
        #     row, 0] * [row])) for row in range(df.shape[0])), axis=0)
    ...
    def instance_norm(self, case):
        if self.renorm:
            ...
            stdev = paddle.sqrt(x=paddle.var(
                x=case, axis=1, keepdim=True, unbiased=False) + 1e-05)
            case /= stdev
            return case.cast('float32')
        else:
            return case.cast('float32')
