...
class Seasonality(Analyzer):
    ...
    def _seasonality(
            self,
            X: pd.DataFrame,
            period_dict: dict={}, ) -> Union[Any, pd.Series]:
        """
        ...
        for col in period_dict:
            if period_dict[col] is not None and period_dict[col] * 2 < len(X[
                    col]):
    ...
    def _period(self, X: pd.DataFrame) -> Union[Any, pd.Series]:
        """
        Compute the seasonality period of given columns
    ...
    def _cal_period(self, X: pd.Series, col: str):
        ...
        #adjust confidence
        interval = [
            confident[lag][1] - acf_values[lag]
...
class Acf(Analyzer):
    ...
    def __init__(self, nlags: int=300, alpha: float=0.05, **kwargs):
        super().__init__(**kwargs)
        raise_if(
    ...
    def plot(self) -> "pyplot":
        ...
        for i in range(0, columns_num):
            ...
            ax[i, 0].fill_between(
                np.arange(1, self.nlags + 1),
                confident, [-x for x in confident],
                alpha=0.25)
            ax[i, 0].set_title(col_name)
...
class Correlation(Analyzer):
    ...
    def analyze(self, X: pd.DataFrame) -> Union[Any, pd.Series]:
        """
        Compute the correlation values of given columns.
