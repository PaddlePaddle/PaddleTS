...
class _Anomaly(paddle.nn.Layer):
    ...
    def __init__(self,
        ...
        self.projection = paddle.nn.Linear(
            d_model, c_out, bias_attr=True)  #bias
...
class AnomalyTransformer(AnomalyBaseModel):
    ...
    def predict(
            ...
            train_dataset: TSDataset=None, ) -> TSDataset:
        ...
        thre_dataloader = self._init_predict_dataloader(
            test_dataset, sampling_stride=self.in_chunk_len)
    ...
    def _predict(
        ...
        for batch_nb, data in enumerate(dataloader):
            ...
            cri = U.series_prios_energy(
                [output, series, prior, _],
                loss,
                temperature=self._temperature,
                win_size=self.in_chunk_len)
    ...
    def predict_score(
        ...
        Returns:
            TSDataset.
        """
        dataloader = self._init_predict_dataloader(
            tsdataset, sampling_stride=self.in_chunk_len)
    ...
    def _fit(self,
        ...
        for epoch_idx in range(self._max_epochs):
            ...
            self._adjust_lr(
                self._optimizer, epoch_idx,
                self._optimizer_params['learning_rate'])  # update lr
    ...
    def _train_batch(
        ...
        loss1, loss2, for_loss_one = self._compute_loss(
            output_list,
            y,
            criterion=self._criterion,
            win_size=self.in_chunk_len,
        ...
        batch_logs = {"batch_size": y.shape[0], "loss": avg_loss}
        return batch_logs
