...
class ReprForecasting(StackingEnsembleBase, BaseModel):
    ...
    def _set_params(self, estimators):
        ...
        if estimators[0][0] is TS2Vec:
            if "sliding_len" in self._encode_params:
                raise_if(
                    self._encode_params["sliding_len"] != self._in_chunk_len,
    ...
    def fit(self, tsdataset: TSDataset) -> None:
        """
        fit 
        ...
        if tsdataset._observed_cov and tsdataset.observed_cov.end_time > tsdataset.target.end_time:
            tsdataset._observed_cov, _ = tsdataset._observed_cov.split(
                target_len)
    ...
    def _generate_fit_meta_data(
            ...
            valid_data: TSDataset=None) -> Tuple[np.ndarray, np.ndarray]:
        """
        ...
        # get samples
        samples = DataAdapter().to_sample_dataset(
            encode_tsdataset,
            in_chunk_len=1,
    ...
    def _generate_predict_meta_data(
            self, tsdataset: TSDataset) -> Tuple[np.ndarray, np.ndarray]:
        """
    ...
    def save(self,
             path: str,
             repr_forecaster_file_name: str="repr-forecaster-partial.pkl"
             ) -> None:
        """
