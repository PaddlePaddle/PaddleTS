...
def instance_contrastive_loss(repr1: paddle.Tensor,
...
def temporal_contrastive_loss(repr1: paddle.Tensor,
                              repr2: paddle.Tensor) -> paddle.Tensor:
    ...
    repr = paddle.concat(
        [repr1, repr2], axis=1)  # [batch_size, 2 * seq_len, d_model]
    sim = paddle.matmul(
        repr, repr, transpose_y=True)  # [seq_len, 2 * seq_len, 2 * seq_len]
    logits = paddle.tril(
        sim,
        diagonal=-1)[:, :, :-1]  # [batch_size, 2 * seq_len, 2 * seq_len - 1]
    logits += paddle.triu(sim, diagonal=1)[:, :, 1:]
    logits = -1. * F.log_softmax(logits, axis=-1)
...
def hierarchical_contrastive_loss(repr1: paddle.Tensor,
                                  repr2: paddle.Tensor,
                                  alpha: float=0.5,
                                  temporal_unit: int=0) -> paddle.Tensor:
