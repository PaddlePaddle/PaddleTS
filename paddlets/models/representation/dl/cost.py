...
from paddlets.models.representation.dl._cost.losses import (
    time_contrastive_loss, frequency_contrastive_loss, convert_coefficient)
...
class _CoSTModule(paddle.nn.Layer):
    ...
    def __init__(self,
        ...
        self._feat_extractor = TSEncoder(
            ...
            seq_len=segment_size)  # feature extractor
    ...
    def _dequeue_and_enqueue(self, keys: paddle.Tensor):
    ...
    def forward(
            self,
            X: paddle.Tensor,
            mask: Optional[str], ) -> paddle.Tensor:
        """Forward.
        ...
        trend_loss = time_contrastive_loss(trend1, trend2,
                                           paddle.assign(self._queue.detach()),
                                           self._temperature)
...
class CoST(ReprBaseModel):
    ...
    def _update_fit_params(
            self,
            train_tsdataset: List[TSDataset], ) -> Dict[str, Any]:
        ...
        fit_params = {"input_dim": input_dim}
        return fit_params
    ...
    def _init_fit_dataloader(
        ...
        dataset.samples = samples * (
            1 if len(samples) >= self._batch_size else
            int(np.ceil(self._batch_size / len(samples))))
        return data_adapter.to_paddle_dataloader(
            dataset, self._batch_size, drop_last=True)
    ...
    def _train_batch(
        ...
        self._network._avg_proj.update_parameters(self._network._out_proj)
        return batch_logs
