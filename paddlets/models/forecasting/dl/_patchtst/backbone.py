...
class PatchTST_backbone(paddle.nn.Layer):
    def __init__(self,
        ...
        if padding_patch == 'end':
            self.padding_patch_layer = paddle.nn.Pad1D(
                padding=(0, stride), mode="replicate")
            patch_num += 1
    ...
    def create_pretrain_head(self, head_nf, vars, dropout):
        return paddle.nn.Sequential(
            ...
            paddle.nn.Conv1D(
                in_channels=head_nf, out_channels=vars, kernel_size=1))
    ...
    def __init__(self, individual, n_vars, nf, target_window, head_dropout=0):
        ...
        if self.individual:
            ...
            for i in range(self.n_vars):
                ...
                self.linears.append(
                    paddle.nn.Linear(
                        in_features=nf, out_features=target_window))
...
class TSTiEncoder(paddle.nn.Layer):
    ...
    def forward(self, x) -> paddle.Tensor:
        ...
        x = self.W_P(x)
        u = paddle.reshape(
            x=x, shape=(x.shape[0] * x.shape[1], x.shape[2], x.shape[3]))
...
class TSTEncoderLayer(paddle.nn.Layer):
    def __init__(self,
        ...
        if 'batch' in norm.lower():
            self.norm_attn = paddle.nn.Sequential(
                Transpose(0, 2, 1),
            ...
            self.norm_attn = paddle.nn.LayerNorm(
                normalized_shape=d_model,
                epsilon=1e-05,
                weight_attr=None,
                bias_attr=None)
        self.ff = paddle.nn.Sequential(
            paddle.nn.Linear(
                in_features=d_model, out_features=d_ff, bias_attr=bias),
        ...
        if 'batch' in norm.lower():
            self.norm_ffn = paddle.nn.Sequential(
                Transpose(0, 2, 1),
                paddle.nn.BatchNorm1D(
                    num_features=d_model,
                    momentum=1 - 0.1,
            ...
            self.norm_ffn = paddle.nn.LayerNorm(
                normalized_shape=d_model,
                epsilon=1e-05,
                weight_attr=None,
                bias_attr=None)
    ...
    def forward(self,
                src: paddle.Tensor,
                prev: Optional[paddle.Tensor]=None,
                key_padding_mask: Optional[paddle.Tensor]=None,
                attn_mask: Optional[paddle.Tensor]=None) -> paddle.Tensor:
        if self.pre_norm:
...
class _MultiheadAttention(paddle.nn.Layer):
    def __init__(self,
        ...
        self.W_Q = paddle.nn.Linear(
            in_features=d_model, out_features=d_k * n_heads, bias_attr=qkv_bias)
        self.W_K = paddle.nn.Linear(
            in_features=d_model, out_features=d_k * n_heads, bias_attr=qkv_bias)
        self.W_V = paddle.nn.Linear(
            in_features=d_model, out_features=d_v * n_heads, bias_attr=qkv_bias)
        self.res_attention = res_attention
        ...
        self.to_out = paddle.nn.Sequential(
            paddle.nn.Linear(
                in_features=n_heads * d_v, out_features=d_model),
            paddle.nn.Dropout(p=proj_dropout))
    ...
    def forward(self,
        ...
        q_s = self.W_Q(Q).reshape([bs, -1, self.n_heads, self.d_k]).transpose(
            [0, 2, 1, 3])
        k_s = self.W_K(K).reshape([bs, -1, self.n_heads, self.d_k]).transpose(
            perm=[0, 2, 3, 1])
        v_s = self.W_V(V).reshape([bs, -1, self.n_heads, self.d_v]).transpose(
            [0, 2, 1, 3])
        ...
        output = x.transpose(perm=perm_2).reshape(
    ...
    def forward(self,
        ...
        if key_padding_mask is not None:
            attn_scores.masked_fill_(
                key_padding_mask.unsqueeze(axis=1).unsqueeze(axis=2), -np.inf)
