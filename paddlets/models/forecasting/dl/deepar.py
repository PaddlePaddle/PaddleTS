...
class _DeepAR(nn.Layer):
    ...
    def __init__(
        ...
        self._rnn = getattr(nn, rnn_type)(self._input_size,
                                          hidden_dim,
        ...
        self.output_projector = nn.Linear(
            hidden_dim, likelihood_model.num_params * target_dim)
    ...
    def _build_input(
        ...
        """
        target_roll = paddle.roll(
            target, shifts=1, axis=1)  # [batch_size, seq_len, target_dim]
    ...
    def _decode_direct(self,
        ...
        output = self.output_projector(
            decoder_output
        )  # model output, [batch_size*num_samples, seq_len, target_dim* num_args]
    ...
    def _decode_regressive_by_mean(self,
        ...
        for step in range(num_steps):
            ...
            current_target_params, hidden_state = self._decode_direct(
                x, hidden_state)

            # 2> sampling num_samples each step
            # for each path, sample once, shape: [100, batch_size, 1, target_dim]
            distr_params = self._likelihood_model.output_to_params(
        ...
        quantiles_output = paddle.quantile(
            quantiles_output,
            list(np.linspace(
                0, 1, OUTPUT_QUANTILE_NUM, endpoint=True)),
    ...
    def _decode_regressive_by_sampling(self,
        ...
        if isinstance(hidden_state, tuple):  #for LSTM
            h, c = hidden_state
            hidden_state = (h.repeat_interleave(self._num_samples, 1),
                            c.repeat_interleave(self._num_samples, 1))
        else:  #for GRU
        ...
        for step in range(num_steps):
            ...
            current_pred_sample = self._likelihood_model.sample(
                current_target_params, 1)
            ...
            current_pred_mean = current_pred_sample.reshape(
                [-1, self._num_samples, self._target_dim]).median(axis=1)

            # 3> make sampling prediction as current step's output and the next step's input
            prediction_output.append(
                current_pred_mean)  # [batch_size, target_dim]
    ...
    def _encoder(self, data: Dict[str, paddle.Tensor]) -> paddle.Tensor:
        """
        ...
        input_tensor = self._build_input(
            past_target,
            past_known_cov)  # encoder do not need `first_target_replace`
        #2> encode by rnn
        _, hidden_state = self._rnn(
            inputs=input_tensor)  # seq_len: in_chunk_len -1
        return hidden_state  # for LSTM, (h, c)

    def _decoder(self,
                 data: Dict[str, paddle.Tensor],
                 hidden_state: paddle.Tensor) -> paddle.Tensor:
        """
        ...
        if not self.predicting:  # for training and validation phase
            ...
            if self._regression_mode == "mean":
                quantiles_output, prediction_output = self._decode_regressive_by_mean(
                    input_tensor, hidden_state)
            else:
                quantiles_output, prediction_output = self._decode_regressive_by_sampling(
                    input_tensor, hidden_state)
