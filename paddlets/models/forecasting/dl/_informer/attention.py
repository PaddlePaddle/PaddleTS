...
class CrossAttention(paddle.nn.Layer):
    ...
    def __init__(
        ...
        raise_if_not(self._head_dim * num_heads == embed_dim,
    ...
    def forward(self,
        ...
        if attn_mask is not None:
            ...
            attn_mask = paddle.tile(attn_mask[None, None, :, :][
        ...
        context = paddle.reshape(
            context, shape=[0, 0, self._num_heads * self._head_dim])
...
class ProbSparseAttention(paddle.nn.Layer):
    ...
    def _compute_prob_qk(self,
                         query: paddle.Tensor,
                         key: paddle.Tensor,
                         sample_k: int,
                         n_top: int) -> Tuple[paddle.Tensor, paddle.Tensor]:
        ...
        K_expand = paddle.tile(key[:, :, None, :, :], [1, 1, L_Q, 1, 1])
        ...
        Q_sample = paddle.take_along_axis(
            query, M_top_index[:, :, :, None], axis=-2)
    ...
    def forward(self,
        ...
        if attn_mask is not None:
            ...
            attn_mask = paddle.tile(attn_mask[None, None, :, :][
                batch_size, self._num_heads, 1, 1])
            attn_mask = paddle.take_along_axis(
                attn_mask, index[:, :, :, None], axis=-2)
            scores = scores + attn_mask
        ...
        context = paddle.put_along_axis(
            context, index[:, :, :, None], values, axis=-2)
