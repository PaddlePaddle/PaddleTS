...
import numpy as np
from paddlets.models.forecasting.dl.PatchTST_backbone import PatchTST_backbone
from paddlets.models.forecasting.dl.PatchTST_layer import series_decomp
...
class _PatchTSTModule(paddle.nn.Layer):
    ...
    def forward(self, x):
        x = x['past_target']
        # #x = np.load('/ssd2/sunting13/ts/PatchTST/PatchTST_supervised/ts.npy')
        # x = np.load('/ssd2/sunting13/ts/PatchTST/PatchTST_supervised/test_x.npy').astype('float32')
...
class PatchTSTModel(PaddleBaseModelImpl):
    ...
    #     )
    #     super(PatchTSTModel, self)._check_params()
    ...
    def _init_network(self) -> paddle.nn.Layer:
        ...
        return _PatchTSTModule(
            c_in=self._fit_params["target_dim"] + self._fit_params["known_cov_dim"] + self._fit_params["observed_cov_dim"],
