...
class TemporalFusionTransformer(nn.Layer):
    ...
    def __init__(
        ...
        self._historical_ts_transform = InputChannelEmbedding(
            ...
            _historical_categorical_cardinalities,
        ...
        self._static_selection = VariableSelectionNetwork(
            ...
            dropout=self.
            _dropout) if self._static_num_dim + self._static_cat_dim else None

        self._historical_ts_selection = VariableSelectionNetwork(
            input_dim=self._state_size,
            num_inputs=self._num_historical_numeric +
            self._num_historical_categorical,
            hidden_dim=self._state_size,
        ...
        self._static_encoder_sequential_cell_init = copy.deepcopy(
            static_covariate_encoder)
        self._static_encoder_sequential_state_init = copy.deepcopy(
            static_covariate_encoder)

        # ============================================================
        # Locality Enhancement with Sequence-to-Sequence processing
        # ============================================================
        self._past_lstm = nn.LSTM(
            input_size=self._state_size,
            hidden_size=self._state_size,
            num_layers=self._lstm_layers,
            dropout=self._dropout)
        self._future_lstm = nn.LSTM(
            input_size=self._state_size,
            hidden_size=self._state_size,
            num_layers=self._lstm_layers,
            dropout=self._dropout)
        self._post_lstm_gating = GateAddNorm(
            input_dim=self._state_size, dropout=self._dropout)
        ...
        self._multihead_attn = InterpretableMultiHeadAttention(
            embed_dim=self._state_size, num_heads=self._attention_heads_num)
        self._post_attention_gating = GateAddNorm(
            input_dim=self._state_size, dropout=self._dropout)
        ...
        self._pos_wise_ff_grn = GatedResidualNetwork(
            ...
            output_dim=self._state_size,
            dropout=self._dropout)
        self._pos_wise_ff_gating = GateAddNorm(
            input_dim=self._state_size, dropout=None)
    ...
    def apply_temporal_selection(
            ...
            temporal_selection_module: VariableSelectionNetwork) -> Tuple[
        ...
        """
        num_samples, num_temporal_steps, _ = paddle.shape(
        ...
        if static_selection_signal is not None:
            ...
            time_distributed_context = self.replicate_along_time(
                static_signal=static_selection_signal,
        ...
        temporal_selection_weights = temporal_selection_weights.squeeze(
            -1).reshape([num_samples, num_temporal_steps, -1])
        return temporal_selection_output, temporal_selection_weights

    @staticmethod
    def replicate_along_time(static_signal: paddle.Tensor,
                             time_steps: int) -> paddle.Tensor:
        """
        ...
        """
        time_distributed_signal = paddle.tile(
            static_signal.unsqueeze(1), [1, time_steps, 1])
        return time_distributed_signal

    @staticmethod
    def stack_time_steps_along_batch(
            temporal_signal: paddle.Tensor) -> paddle.Tensor:
        """
    ...
    def transform_inputs(
        ...
        static_rep = self._static_transform(
            x_numeric=batch.get('static_cov_numeric', empty_tensor).squeeze(
                [1]),
            x_categorical=batch.get('static_cov_categorical',
                                    empty_tensor).squeeze([1]))
        ...
        future_ts_rep = self._future_ts_transform(
            x_numeric=batch.get('known_cov_numeric',
                                empty_tensor)[:, self._in_chunk_len:, :],
            x_categorical=batch.get('known_cov_categorical',
                                    empty_tensor)[:, self._in_chunk_len:, :])
        return future_ts_rep, historical_ts_rep, static_rep

    def get_static_encoders(
            self, selected_static: paddle.Tensor) -> Tuple[paddle.Tensor, ...]:
        """
        ...
        c_seq_hidden = self._static_encoder_sequential_state_init(
            selected_static)
    ...
    def apply_sequential_processing(self,
        ...
        lstm_input = paddle.concat(
            [selected_historical, selected_future], axis=1)
        ...
        lstm_output = paddle.concat(
            [past_lstm_output, future_lstm_output], axis=1)

        # perform gating to the recurrent output signal, using a residual connection to input of this block
        gated_lstm_output = self._post_lstm_gating(
            lstm_output, residual=lstm_input)
        return gated_lstm_output

    def apply_static_enrichment(
            self,
            gated_lstm_output: paddle.Tensor,
            static_enrichment_signal: paddle.Tensor) -> paddle.Tensor:
        """
        ...
        if static_enrichment_signal is not None:
            # replicate the selection signal along time
            time_distributed_context = self.replicate_along_time(
                static_signal=static_enrichment_signal,
                time_steps=num_temporal_steps)
        ...
        flattened_gated_lstm_output = self.stack_time_steps_along_batch(
            gated_lstm_output)
    ...
    def apply_self_attention(
            ...
            num_future_steps: int) -> Tuple[paddle.Tensor, paddle.Tensor]:
        """
        ...
        mask = paddle.concat(
            [
                paddle.zeros([output_sequence_length, num_historical_steps]),
                paddle.triu(
                    paddle.ones(
