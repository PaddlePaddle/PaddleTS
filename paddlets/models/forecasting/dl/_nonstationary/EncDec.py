...
class ConvLayer(paddle.nn.Layer):
    def __init__(self, c_in):
        ...
        self.downConv = paddle.nn.Conv1D(
            in_channels=c_in,
            out_channels=c_in,
            kernel_size=3,
            padding=2,
...
class EncoderLayer(paddle.nn.Layer):
    def __init__(self,
        ...
        self.activation = (paddle.nn.functional.relu if activation == 'relu'
                           else paddle.nn.functional.gelu)
    ...
    def forward(self, x, attn_mask=None, tau=None, delta=None):
        ...
        y = x = self.norm1(x)
        y = self.dropout(
            self.activation(self.conv1(y.transpose(perm=[0, 2, 1]))))
...
class Encoder(paddle.nn.Layer):
    def __init__(self, attn_layers, conv_layers=None, norm_layer=None):
        ...
        self.conv_layers = paddle.nn.LayerList(
            sublayers=conv_layers) if conv_layers is not None else None
        self.norm = norm_layer

    def forward(self, x, attn_mask=None, tau=None, delta=None):
        attns = []
        if self.conv_layers is not None:
            for i, (attn_layer, conv_layer
                    ) in enumerate(zip(self.attn_layers, self.conv_layers)):
    ...
    def forward(self,
        ...
        x = x + self.dropout(
            self.cross_attention(
                x, cross, cross, attn_mask=cross_mask, tau=tau, delta=delta)[0])
        y = x = self.norm2(x)
        y = self.dropout(
            self.activation(self.conv1(y.transpose(perm=[0, 2, 1]))))
        y = self.dropout(self.conv2(y).transpose(perm=[0, 2, 1]))
