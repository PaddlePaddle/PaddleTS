...
class ShapExplainer(BaseExplainer):
    ...
    def __init__(
        ...
        raise_if(shap_method not in ['kernel', 'deep'],
                 'Only support kernel shap and deep shap!')
        raise_if_not(
            isinstance(background_data.freq, str),
            'Only support timeindex data!')
        raise_if(
            len(background_data.get_target().columns) > 1,
            'Only support univariate output!')
        ...
        if issubclass(type(model), Pipeline) and shap_method != 'deep':
            ...
            raise_log(
                ValueError(
                    f"The model type ({type(model)}) is not supported by %s explainer."
                    % shap_method))

        raise_if(shap_method == 'deep' and issubclass(
            type(model), DeepARModel),
                 "DeepAR is not supported by deep explainer.")
        # Judge whether it is probability prediction
        if hasattr(_model_obj, "_output_mode"):
            raise_if(
                _model_obj._output_mode == 'quantiles',
                'Only support point prediction but not probability prediction!')
        ...
        if background_sample_number:
            ...
            if shap_method == 'kernel':
                self.new_background_data = self.new_background_data.sample(
                    background_sample_number, random_state=seed)
            elif shap_method == 'deep':
                random.seed(seed)
                choice = random.sample(
                    range(self.new_background_data['past_target'].shape[0]),
                    background_sample_number)
                for k in self.new_background_data:
                    self.new_background_data[k] = self.new_background_data[k][
                        choice]
        ...
        if shap_method == 'kernel':
            ...
            if use_paddleloader and not issubclass(type(self._model), Pipeline):
                ...
                self.explainer = self._ShapMethod[shap_method](
                    model=self._wrapper_predict,
                    data=self.new_background_data,
                    **kwargs)
        elif shap_method == 'deep':
            self.explainer = self._ShapMethod[shap_method](
                model=self._model, data=self.new_background_data)
    ...
    def _wrapper_paddle_predict(self, df: pd.DataFrame) -> np.ndarray:
        ...
        data_loader = PaddleDataLoader(
            dataset=paddle_ds, batch_size=len(paddle_ds))
    ...
    def _ts_to_tensor(self, ts: TSDataset,
                      is_explain: bool=False) -> Dict[str, paddle.Tensor]:
        """
        ...
        _samples = _init_dataloaders(ts)[
            0] if not is_explain else _init_dataloaders(ts)
        tensor_sample = {}
        for _sample in _samples:
            ...
            for k in _sample.keys():
                if k not in tensor_sample:
                    ...
                    tensor_sample[k] = paddle.concat(
                        [tensor_sample[k], _sample[k]], axis=0)
    ...
    def explain(
        ...
        if self.shap_method == 'kernel':
            ...
            new_foreground_data = self._ts_to_tensor(
                foreground_data, is_explain=True)
            sample_len = new_foreground_data['past_target'].shape[0]

        raise_if((sample_start_index >= sample_len) or (
            sample_start_index < -sample_len
        ...
        if self.shap_method == 'kernel':
            new_foreground_data = new_foreground_data.iloc[
                sample_start_index:sample_start_index + sample_num, :]
            shap_value = self.explainer.shap_values(
                new_foreground_data, nsamples=nsamples, **kwargs)
            ...
            shap_value_np = np.zeros(
                (self._out_chunk_len, sample_len - sample_start_index,
                 self._in_chunk_len + self._out_chunk_len, len(unique_cols)))
            for sample_index in range(sample_len - sample_start_index):
                ...
                for col_index, col in enumerate(columns):
                    ...
                    for out_index in range(self._out_chunk_len):
                        ...
                        shap_value_np[out_index, sample_index, lag_index,
                                      unique_cols.index(name)] = value

        elif self.shap_method == 'deep':
            for k in new_foreground_data.keys():
                new_foreground_data[k] = new_foreground_data[k][
                    sample_start_index:sample_start_index + sample_num, :]
            ...
            for k, v in self.deep_used_cols.items():
                ...
                shap_value_k = np.stack(
                    [tensor_dict[k] for tensor_dict in shap_value])
                shap_value_np[:, :, :shap_value_k.shape[2],
                              v_index] += shap_value_k

        self.unique_cols = unique_cols
        self.out_cols = list(range(1, self._out_chunk_len + 1))
        self.in_cols = list(
            range(-self._in_chunk_len + 1, self._out_chunk_len + 1))
    ...
    def get_explanation(
        ...
        """
        raise_if(out_chunk_index < 1,
                 'out_chunk_index should be a positive integer.')
    ...
    def plot(
        ...
        for key in method:
            _plot_method[key](shap_value_np,
    ...
    def force_plot(
        ...
        Returns:
            None
        """
        out_chunk_indice = [out_chunk_indice] if isinstance(
            out_chunk_indice, int) else out_chunk_indice

        raise_if(
            min(out_chunk_indice) < 1, 'out_chunk_indice must be more than 0!')
        ...
        for i in range(self.shap_value.shape[2]):
            for j in range(self.shap_value.shape[3]):
                feature_names.append('%s:%s_lag_%d' % (
                    self.unique_cols[j],
                    self.key_feature[self.unique_cols[j]].split('_')[0],
                    i - self._in_chunk_len + 1))
        if self.shap_method == 'kernel':
            _sample = self.new_foreground_data.iloc[sample_index:sample_index +
                                                    1, :]
        elif self.shap_method == 'deep':
            values = []
            cols = []
            for k, tensor in self.new_foreground_data.items():
                ...
                for time_step in range(tensor.shape[1]):
                    for fea_index in range(tensor.shape[2]):
                        cols.append('%s:%s_lag_%d' %
                                    (self.deep_used_cols[k][fea_index],
                                     self.key_feature[self.deep_used_cols[k][
                                         fea_index]].split('_')[0],
                                     time_step - self._in_chunk_len + 1))
                        values.append(tensor[sample_index, time_step,
                                             fea_index])
        ...
        for i, index in enumerate(out_chunk_indice):
            ...
            shap.force_plot(self.explainer.expected_value[index], sv, _sample,
                            **kwargs)
    ...
    def _get_feature_name(self, ts: TSDataset) -> Dict[str, List[str]]:
        ...
        if observed:
            ...
            if not observed_df_cat.empty:
                tensor_feature_name['observed_cov_categorical'].extend(
                    observed_df_cat.columns)
            if not observed_df_num.empty:
                tensor_feature_name['observed_cov_numeric'].extend(
                    observed_df_num.columns)
        if known:
            ...
            if not known_df_cat.empty:
                tensor_feature_name['known_cov_categorical'].extend(
                    known_df_cat.columns)
            if not known_df_num.empty:
                tensor_feature_name['known_cov_numeric'].extend(
                    known_df_num.columns)
        if static:
