...
class TFTExplainer(TFTModel):
    ...
    def _update_fit_params(
        ...
        """
        fit_params = super()._update_fit_params(train_tsdataset,
                                                valid_tsdataset)
        ...
        self.mapping = {
            'Static Weights': {
                'arr_key': 'static_weights',
                'feat_names': self.static_cols
            },
            'Historical Weights': {
                'arr_key': 'historical_selection_weights',
                'feat_names': self.history_cols
            },
            'Future Weights': {
                'arr_key': 'future_selection_weights',
                'feat_names': self.future_cols
            },
        }
        return fit_params
    ...
    def _display_explanations(
        ...
        # Selection Weights
        # ========================
        self._display_selection_weights_stats(
            outputs_dict=explain_data,
            prctiles=weights_prctile,
            mapping=self.mapping)
        self._display_sample_wise_selection_stats(
            weights_arr=explain_data['static_weights'],
            observation_index=observation_index,
            feature_names=self.static_cols,
            top_n=top_feature_num,
            title='Static Features')
        self._display_sample_wise_selection_stats(
            weights_arr=explain_data['historical_selection_weights'],
            observation_index=observation_index,
            ...
            rank_stepwise=True)
        self._display_sample_wise_selection_stats(
            weights_arr=explain_data['future_selection_weights'],
            observation_index=observation_index,
        ...
        if len(weights_prctile) > 1:  # only for backtest scenario 
            for horizon in horizons:
                self._display_attention_scores(
                    attention_scores=explain_data['attention_scores'],
                    horizons=horizon,
                    prctiles=weights_prctile,
                    unit=unit)
        ...
        if explain_data['attention_scores'].shape[0] > 1:
            self._display_sample_wise_attention_scores(
                attention_scores=explain_data['attention_scores'],
                observation_index=observation_index,
                horizons=horizons,
                unit=unit)
    ...
    def _check_backtest_params(
        ...
        raise_if(
            start < in_chunk_len + skip_chunk_len,
    ...
    def explain_backtest(self,
        ...
        all_observe = data.get_observed_cov() if data.get_observed_cov(
        ) else None
        if start is None:
        ...
        self._check_backtest_params(
            len(all_target), self._in_chunk_len, self._skip_chunk_len, start)
        predict_rounds = math.ceil(
            (len(all_target) - start) / self._sampling_stride)
        ...
        for _ in tqdm(range(predict_rounds), desc="Backtest Progress"):
            ...
            data._observed_cov, _ = all_observe.split(
                index) if all_observe else (None, None)
            rest_len = len(rest)
            if rest_len < predict_window + self._skip_chunk_len:
                if data.known_cov is not None:
                    ...
                    if len(
                            data.known_cov
                    ) - known_index - 1 < predict_window + self._skip_chunk_len:
                        break
        ...
        # visualization
        if display:
            self._display_explanations(
                explain_data=outputs,
    ...
    def _aggregate_weights(self,
        ...
        prctiles_agg = [
        ]  # a list to contain the computation for each percentile
        ...
        agg_df = pd.DataFrame(
            {prctile: aggs
             for prctile, aggs in zip(prctiles, prctiles_agg)})
    ...
    def _display_selection_weights_stats(
        ...
        for name, config in mapping.items():
            ...
            weights_agg = self._aggregate_weights(
                output_arr=outputs_dict[config['arr_key']],
                prctiles=prctiles,
            ...
            display(
                weights_agg.sort_values(
                    [sort_by], ascending=False).style.background_gradient(
                        cmap='viridis'))

    def _display_attention_scores(self,
        ...
        for prctile in prctiles:
            attn_stats[prctile] = np.percentile(
                attention_scores, q=prctile, axis=0)
        ...
        if len(prctiles) == 1:  # in case only a single percentile was configured
            ...
            for horizon in horizons:  # a single line for each horizon
                ...
                ax.plot(
                    x_axis,
                    scores_percentile[horizon - 1],
                    lw=1,
                    label=f"t + {horizon} scores",
                    marker='o')
        else:
            ...
            for prctile, scores_percentile in attn_stats.items(
                ...
                ax.plot(
                    x_axis,
                    scores_percentile[horizons[0] - 1],
                    lw=1,
                    label=f"{prctile}%",
                    marker='o')
    ...
    def _display_sample_wise_attention_scores(self,
                                              attention_scores: np.ndarray,
    ...
    def _display_sample_wise_selection_stats(
        ...
        if is_temporal:
            ...
            weights_df = pd.DataFrame(
                {
                    'weight': sample_weights_trans.mean(axis=1)
                },
                index=feature_names)
        else:
        ...
        weights_df.sort_values(
            'weight', ascending=False).iloc[:top_n].plot.bar(ax=ax)
        ...
        if is_temporal:
            ...
            if historical:
                map_x = {
                    ...
                    for idx, val in enumerate(
                        np.arange(0 - num_temporal_steps, 1))
                }
            else:
                map_x = {
                    idx: val
                    for idx, val in enumerate(
                        np.arange(1, num_temporal_steps + 1))
                }
            ...
            ax.set_xlabel(('Historical'
                           if historical else 'Future') + ' Time-Steps')
