...
def check_model_fitted(model: Trainable, msg: str=None):
    ...
    if isclass(model):
        raise_log(
            ValueError(f"{type(model).__name__}is a class, not an instance."))
    if msg is None:
        msg = ("This %(name)s instance is not fitted yet. Call 'fit' with "
               "appropriate arguments before using this estimator.")
    if not isinstance(model, Trainable):
        raise_log(
            ValueError(f"{type(model).__name__} is not a Trainable Object."))
    ...
    # ML 模型
        ...
        fitted = True if "model" in vars(model) or "_model" in vars(
            model) else False
...
def check_train_valid_continuity(train_data: TSDataset,
                                 valid_data: TSDataset) -> bool:
    """
    ...
    if isinstance(train_index, pd.DatetimeIndex):
        if isinstance(valid_index, pd.DatetimeIndex):
            if isinstance(train_index.freq, pd.offsets.MonthEnd):
                continuious = pd.offsets.MonthEnd(
                    month_delta(train_index[-1], valid_index[
                        0])).freqstr in train_index.freq.freqstr
            elif isinstance(train_index.freq, pd.offsets.Week):
                continuious = pd.offsets.Week(
                    week_delta(train_index[-1], valid_index[
                        0])).freqstr in train_index.freq.freqstr
            else:
                continuious = (valid_index[0] - train_index[-1] ==
...
def split_dataset(dataset: TSDataset, split_point: int) -> TSDataset:
    ...
    all_index = pd.concat(
        [x.to_series() for x in index_list]).index.drop_duplicates()
    max_len = len(all_index)
    split_index = all_index[split_point - 1]

    raise_if(split_point >= max_len,
             "split point should smaller than dataset length")
    raise_if(split_point <= 0, "split point should > 0")
    raise_if_not(
        isinstance(split_point, Integral),
    ...
    if dataset.target:
        ...
        if split_index < target_index[0]:
            ...
            if isinstance(dataset.target.data.index, pd.RangeIndex):
                target_pre, target_after = dataset.target.split(
                    int((split_index - dataset.target.data.index[0]) /
                        dataset.target.data.index.step + 1))
            else:
    ...
    if dataset.known_cov:
        if split_index < known_index[0]:
            ...
            if isinstance(dataset.known_cov.data.index, pd.RangeIndex):
                known_pre, known_after = dataset.known_cov.split(
                    int((split_index - dataset.known_cov.data.index[0]) /
                        dataset.known_cov.data.index.step + 1))
            else:
    ...
    if dataset.observed_cov:
        if split_index < observed_index[0]:
            ...
            if isinstance(dataset.observed_cov.data.index, pd.RangeIndex):
                observed_pre, observed_after = dataset.observed_cov.split(
                    int(((split_index - dataset.observed_cov.data.index[0])) /
                        dataset.observed_cov.data.index.step + 1))
            else:
                observed_pre, observed_after = dataset.observed_cov.split(
                    split_index)
    return (TSDataset(target_pre, observed_pre, known_pre, dataset.static_cov),
            TSDataset(target_after, observed_after, known_after,
                      dataset.static_cov))
...
def repr_results_to_tsdataset(reprs: np.array, dataset: TSDataset) -> TSDataset:
    ...
    reprs_df = pd.DataFrame(
        reprs,
        columns=[f"repr_{i}" for i in range(reprs.shape[-1])],
        index=dataset.get_target().data.index)
...
def plot_anoms(predict_data: TSDataset=None,
    ...
    def plot_anoms_point(ax: plt.Axes, anomaly_data: TSDataset):
        ...
        ax2.plot(
            anomaly_data.target.data.index,
            anom_vals,
            color="r",
            label=anom_label)
    ...
    def plot_anoms_window(ax: plt.Axes, anomaly_data: TSDataset):
        ...
        for k in range(len(splits) - 1):
            if y[splits[k]]:  # If splits[k] is anomalous
                ax.axvspan(
                    t[splits[k]], t[splits[k + 1]], color="purple", alpha=0.5)
    ...
    if predict_data is None:
        ...
        raise_if_not(
            isinstance(predict_data, TSDataset),
            f"origin data type ({type(origin_data)}) must be TSDataset")
    ...
    if origin_data is None:
        ...
        raise_if_not(
            isinstance(origin_data, TSDataset),
            f"origin data type ({type(origin_data)}) must be TSDataset")
...
def build_ts_infer_input(tsdataset: TSDataset,
                         meta_file: str) -> Dict[str, np.ndarray]:
    """
    ...
    try:
        ...
        raise_log(ValueError(f"Load meta file failed! err: {e}"))
    raise_if_not(
        "model_type" in json_data and "size" in json_data and
        "input_data" in json_data,
    ...
    if json_data['model_type'] == 'forecasting':
        ...
        boundary = (len(tsdataset.get_target().data) - 1 +
                    json_data['size']['skip_chunk_len'] +
                    json_data['size']['out_chunk_len'])
    ...
    dataloader = data_adapter.to_paddle_dataloader(
        dataset, len(dataset.samples), shuffle=False)
...
def update_train_results(save_path, score, model_name="", done_flag=True):
    ...
    for key in save_inference_files:
        train_results["models"]["best"][key] = os.path.join(
            "inference", save_inference_files[key])
