...
class Config(object):
    ...
    Args:
        ...
        opts (Dict, optional): Use opts dict to update the key-value pairs of all options.
...
def update_config_dict(
    ...
    if learning_rate:
        dic['lr_scheduler']['lr_cfg']['learning_rate'] = learning_rate
    if batch_size:
    ...
    if opts is not None:
        for key, val in opts.items():
            dic[key] = val
            assert dic[key] == opts[
                key], f'Value of {key} is not the same in dic and opts'
