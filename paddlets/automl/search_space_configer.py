...
from ray.tune.search.sample import Quantized, Normal
...
class SearchSpaceConfiger:
    ...
    def get_default_search_space(self, paddlets_estimator):
        ...
        """
        if hasattr(paddlets_estimator,
                   "__mro__") and BaseTransform in paddlets_estimator.__mro__:
            for class_name, config in self.paddlets_default_search_space[
                    "transform"].items():
                if paddlets_estimator.__name__ == class_name:
                    return config
        elif hasattr(
                paddlets_estimator,
                "__mro__") and PaddleBaseModel in paddlets_estimator.__mro__:
            for class_name, config in self.paddlets_default_search_space[
                    "models"]["dl"]["paddlepaddle"].items():
                if paddlets_estimator.__name__ == class_name:
                    return config
        elif hasattr(paddlets_estimator,
                     "__mro__") and MLBaseModel in paddlets_estimator.__mro__:
            for class_name, config in self.paddlets_default_search_space[
                    "models"]["ml"].items():
                if paddlets_estimator.__name__ == class_name:
                    return config
            ...
            for step in paddlets_estimator.steps:
                ...
                for class_name, config in {
                        ** self.paddlets_default_search_space["transform"], **
                        self.paddlets_default_search_space["models"]["dl"][
                            "paddlepaddle"], **
                        self.paddlets_default_search_space["models"]["ml"]
                }.items():
                    if e.__name__ == class_name:
                        config_dict[e.__name__ + "-" + str(
                            estimator_index)] = config
                        founded = True
                        break
                if founded is False:
            ...
            if self._sp_empty(config_dict):
                raise NotImplementedError(
                    f"search space is empty, sp: {self.search_space_to_str(config_dict)}"
                )
            return config_dict
    ...
    def _param_search_space_to_str(self, sp):
        ...
        if not hasattr(sp, "sampler"):
            ...
            if isinstance(sp, Categorical):
                return RAY_SAMPLE[sp.__class__][
                    sp.sampler.__class__] + "(" + sp.domain_str + ")"
            else:
                if isinstance(sp.sampler, Normal):
                    return RAY_SAMPLE[sp.__class__][
                        sp.sampler.__class__] + "(" + str(
                            sp.sampler.mean) + ", " + str(sp.sampler.sd) + ")"
                else:
                    return RAY_SAMPLE[sp.__class__][
                        sp.sampler.__class__] + "(" + str(
                            sp.lower) + ", " + str(sp.upper) + ")"
    ...
    def _to_pretty_str(self, sp_dict):
        ...
        return json.dumps(
            sp_dict, indent="\t").replace('",', ",").replace(
                ': "', ": ").replace('"\n', '\n')
            ...
            "models": {
                "dl": {
                    "paddlepaddle": {
                        ...
                        "RNNBlockRegressor": {
                            "rnn_type_or_module":
                            choice(["SimpleRNN", "LSTM", "GRU"]),
                            "fcn_out_config": choice(
                                [[16], [32], [64], [128], [256], [16] * 2,
                                 [32] * 2, [64] * 2, [128] * 2, [256] * 2,
                                 [16] * 3, [32] * 3, [64] * 3, [128] * 3,
                                 [256] * 3]),
                            "hidden_size": qrandint(
                                32, 512, q=32),
                        ...
                        "NBEATSModel": {
                            ...
                            "layer_widths": qrandint(
                                32, 512, q=32),
                            "expansion_coefficient_dim": qrandint(
                                32, 512, q=32),
                        ...
                        "NHiTSModel": {
                            ...
                            "activation": choice([
                                "ReLU", "PReLU", "ELU", "Softplus", "Tanh",
                                "SELU", "LeakyReLU", "Sigmoid", "GELU"
                            ]),
                        ...
                        "TransformerModel": {
                            ...
                            "dim_feedforward": qrandint(
                                32, 512, q=32),
                            "activation": choice(["relu", "gelu"]),
                            "dropout_rate": quniform(
                                0, 0.5, q=0.05),
                            "d_model": qrandint(
                                32, 512, q=32),
                            "batch_size": qrandint(
