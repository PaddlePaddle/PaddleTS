...
from paddlets.metrics import (MetricContainer, LogLoss, Metric, MSE, MAE,
                              QuantileLoss, ACC, Precision, Recall, F1)
...
class TestMetrics(TestCase):
    ...
    def test_LogLoss(self):
        ...
        for schema in ret:
            self.assertAlmostEqual(
                ret[schema], expect_output[schema], delta=1.)
    ...
    def test_prob(self):
        ...
        for tgt in ["target", "target_2"]:
            for q in [0.1, 0.5, 0.9]:
                self.assertAlmostEqual(q_loss_ret[tgt][q],
                                       expect_q_loss_ret[tgt][q])
    ...
    def test_ACC(self):
        ...
        df = pd.DataFrame(
            y_true[1:5],
            index=pd.date_range(
                "2022-01-02", periods=periods, freq="1D"),
            columns=["label"])
...
class TestMetricContainer(TestCase):
    ...
    def test_call(self):
        ...
        """
        # case1
        contrainer = MetricContainer(metrics=["mse", "mae"], prefix="val_")
        ...
        for schema in ret:
            self.assertAlmostEqual(
                ret[schema], expect_output[schema], delta=1e-5)
