...
class TestTimeSeries(TestCase):
    ...
    def test_load_from_dataframe(self):
        ...
        ts1 = TimeSeries.load_from_dataframe(
            data=sample1, value_cols=['a'], time_col='f', freq='12H')
        ...
        self.assertEqual(ts1.data.shape, (200, 1))
        #case9
        sample1 = pd.Series(np.random.randn(200), name='a')
        ts1 = TimeSeries.load_from_dataframe(data=sample1)
        self.assertEqual(ts1.freq, 1)
        self.assertEqual(ts1.data.shape, (200, 1))
        sample1 = pd.Series(
        ...
        ts1 = TimeSeries.load_from_dataframe(
            data=sample1, freq=2, dtype=np.int64)
        self.assertEqual(ts1.freq, 2)
        self.assertEqual(ts1.data.shape, (100, 1))
        self.assertEqual(ts1.dtypes['a'], np.int64)

        ts1 = TimeSeries.load_from_dataframe(
            data=sample1, freq=2, dtype={'a': np.int64})
        self.assertEqual(ts1.freq, 2)
        self.assertEqual(ts1.data.shape, (100, 1))
        self.assertEqual(ts1.dtypes['a'], np.int64)
    ...
    def test_split(self):
        ...
        self.assertEqual(
            ts1.get_index_at_point(
                "2022-01-08 12:45:23", after=False), 7)
        try:
        ...
        try:
            ...
            self.assertEqual(
                str(e), '`point` (int) should be a valid index in series.')
    ...
    def test_getitem(self):
        ...
        sample2 = pd.DataFrame(np.random.randn(200, 2), columns=['a', 'b'])
    ...
    def test_concat(self):
        ...
        ts4 = TimeSeries.concat(
            [ts2, ts3], axis=1, drop_duplicates=True, keep='last')
        self.assertEqual(ts3.data.shape, (200, 2))
        ...
        sample2 = pd.DataFrame(
            np.random.randn(200, 2),
            index=pd.date_range(
                '2022-07-20', periods=200, freq='1D'),
            columns=['a', 'b'])
        ts2 = TimeSeries.load_from_dataframe(data=sample2)
        ...
        sample2 = pd.DataFrame(
            np.random.randn(200, 2),
            index=pd.date_range(
                '2022-07-30', periods=200, freq='1D'),
            columns=['a', 'b'])
        ts2 = TimeSeries.load_from_dataframe(data=sample2)
        ts3 = TimeSeries.concat([ts1, ts2])  #default axis=0
    ...
    def test_json(self):
        ...
        sample1 = pd.DataFrame(
            np.random.randn(200, 3), columns=['a', 'b', '测试列'])
        ts1 = TimeSeries.load_from_dataframe(data=sample1)
        json_data = ts1.to_json()
        ts2 = TimeSeries.load_from_json(json_data)
        self.assertEqual(ts1.data.shape, ts2.data.shape)
...
class TestTSDataset(TestCase):
    ...
    def test_init(self):
        ...
        """
        #case1
        tsdataset = TSDataset(self.target, self.observed_cov, self.known_cov,
                              self.static_cov)
    ...
    def test_load_from_dataframe(self):
        ...
        sample1['id'] = pd.Series([0] * 100 + [1] * 100, name='id')
        ...
        for tsdataset in tsdatasets:
            self.assertEqual(
                list(tsdataset.get_static_cov().keys()), ['s', 'id'])
    ...
    def test_split(self):
        ...
        self.assertEqual(test.get_observed_cov().data.shape,
                         (200 - start_num, 2))
        ...
        self.assertEqual(train.get_observed_cov().data.shape,
                         (200 * start_ratio, 2))
        self.assertEqual(train.get_known_cov().data.shape, (200, 2))
        self.assertEqual(train.get_static_cov(), {'f': 1, 'g': 2})
        ...
        tsdataset = TSDataset(None, self.observed_cov, self.known_cov,
                              self.static_cov)
        start_num = 120
        with self.assertRaises(ValueError):
            train, test = tsdataset.split(start_num)
    ...
    def test_copy(self):
        ...
        unittest function
        """
        ts1 = TSDataset(self.target, self.observed_cov, self.known_cov,
                        self.static_cov)
        ts2 = ts1
        self.assertTrue(id(ts1) == id(ts2))
    ...
    def test_get_item(self):
        ...
        self.assertEqual(
            list(ts1[['a', 'c1', 'c', 'g']].columns), ['a', 'c1', 'c', 'g'])
        self.assertEqual(
            list(ts1[['g', 'c1', 'c', 'a']].columns), ['g', 'c1', 'c', 'a'])
    ...
    def test_concat(self):
        ...
        sample2 = pd.DataFrame(
            np.random.randn(200, 5),
            index=pd.date_range(
                '2022-06-20', periods=200, freq='1D'),
            columns=['a', 'b', 'c', 'd', 'e'])
        sample2['s'] = pd.Series(
            [1 for i in range(200)],
            index=pd.date_range(
    ...
    def test_plot(self):
        ...
        unittest function
        """
        ts = TSDataset(self.target, self.observed_cov, self.known_cov,
                       self.static_cov)
        ...
        target = TimeSeries.load_from_dataframe(
            pd.DataFrame(
                ...
                index=pd.date_range(
                    "2022-01-01", periods=400, freq="15T"),
                columns=["a1", "a2"]))
        ...
        known_cov = TimeSeries.load_from_dataframe(
            pd.DataFrame(
                ...
                index=pd.date_range(
                    "2022-01-01", periods=500, freq="15T"),
                columns=["b1", "c1"]))
        static_cov = {"f": 1, "g": 2}
    ...
    def test_reindex(self):
        ...
        self.assertEqual(tsdataset1.get_known_cov().to_dataframe()['e'][-1],
                         fill_value)
    ...
    def test_sort_columns(self):
        ...
        tsdataset1 = TSDataset.load_from_dataframe(
            ...
            static_cov_cols='s')
        self.assertEqual(
            list(tsdataset1.columns.keys()), ['a', 'e', 'd', 'c', 'b'])
        tsdataset1.sort_columns()
        self.assertEqual(
            list(tsdataset1.columns.keys()), ['a', 'e', 'b', 'c', 'd'])
