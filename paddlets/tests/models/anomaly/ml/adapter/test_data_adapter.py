...
class TestDataAdapter(unittest.TestCase):
    ...
    def test_to_ml_dataset(self):
        """
        Test DataAdapter.to_ml_dataset()
        """
        # ##############################################
        # case 0 (good case)                           #
        ...
        sample_ds = self._adapter.to_ml_dataset(tsdataset)
        ...
        sample_ds = self._adapter.to_ml_dataset(tsdataset, **param)
        self._compare_tsdataset_and_sample_dataset(
            tsdataset=tsdataset,
            sample_ds=sample_ds,
            param=param
        ...
        succeed = True
        try:
            _ = self._adapter.to_ml_dataset(rawdataset=tsdataset)
        except ValueError:
            succeed = False
        ...
        try:
            # in_chunk_len = 0.
            _ = self._adapter.to_ml_dataset(rawdataset=tsdataset, in_chunk_len=0)
        except ValueError:
            succeed = False
        self.assertFalse(succeed)
        ...
        try:
            # in_chunk_len < 0
            _ = self._adapter.to_ml_dataset(rawdataset=tsdataset, in_chunk_len=-1)
        except ValueError:
            succeed = False
        self.assertFalse(succeed)
        ...
        try:
            # sampling_stride = 0.
            _ = self._adapter.to_ml_dataset(rawdataset=tsdataset, sampling_stride=0)
        except ValueError:
            succeed = False
        self.assertFalse(succeed)
        ...
        try:
            # sampling_stride < 0
            _ = self._adapter.to_ml_dataset(rawdataset=tsdataset, sampling_stride=-1)
        except ValueError:
            succeed = False
        self.assertFalse(succeed)
        ...
        try:
            ...
            _ = self._adapter.to_ml_dataset(rawdataset=tsdataset, in_chunk_len=in_chunk_len)
        except ValueError:
            succeed = False
        self.assertFalse(succeed)
    ...
    def test_to_ml_dataloader(self):
        """
        Test DataAdapter.to_ml_dataloader()
        """
        ################################
        # case 0 (good case)           #
        ...
        sample_ds = self._adapter.to_ml_dataset(tsdataset)
        batch_size = 2
        sample_dataloader = self._adapter.to_ml_dataloader(sample_ds, batch_size)
    ...
    def _build_mock_ts_dataset(
        ...
        # target
        target_raw_data = [i for i in range(target_periods)]
        target_df = pd.DataFrame(
            data=np.array(target_raw_data, dtype=numeric_dtype),
    ...
    def _compare_tsdataset_and_sample_dataset(
        ...
        """
        Given a TSDataset and a built MLDataset, compare if these data are matched.

        Args:
            tsdataset(TSDataset): Raw TSDataset.
            sample_ds(MLDataset): Built MLDataset.
