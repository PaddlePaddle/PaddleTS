...
class TestAnomalyTransformer(TestCase):
    def setUp(self):
        ...
        self.tsdataset4 = TSDataset.load_from_dataframe(
            pd.concat(
                [label, feature], axis=1), feature_cols=['a', 'b'])
        super().setUp()
    ...
    def test_init(self):
        ...
        model = AnomalyTransformer(in_chunk_len=16, **param1)
        ...
        with self.assertRaises(ValueError):
            model = AnomalyTransformer(in_chunk_len=16, **param2)
        ...
        with self.assertRaises(ValueError):
            model = AnomalyTransformer(in_chunk_len=16, **param3)
        ...
        with self.assertRaises(ValueError):
            model = AnomalyTransformer(in_chunk_len=16, **param4)
        ...
        with self.assertRaises(ValueError):
            model = AnomalyTransformer(in_chunk_len=16, **param5)

    def test_init_dataloader(self):
        """unittest function for init dataloader
        """
        model = AnomalyTransformer(in_chunk_len=16, max_epochs=1)
    ...
    def test_init_metrics(self):
        ...
        model = AnomalyTransformer(
            in_chunk_len=16, max_epochs=1, eval_metrics=["mae"])
    ...
    def test_init_callbacks(self):
        ...
        """
        # case1 (patience = 0)
        model = AnomalyTransformer(in_chunk_len=16, patience=0)
        ...
        # case2 (patience > 0)
        model = AnomalyTransformer(in_chunk_len=16, patience=1)
        model._metrics, model._metrics_names, _ = model._init_metrics(["val"])
        ...
        callback = Callback()
        model = AnomalyTransformer(in_chunk_len=16, callbacks=[callback])
        model._metrics, model._metrics_names, _ = model._init_metrics(["val"])
        _, callback_container = model._init_callbacks()
    ...
    def test_predict(self):
        ...
        model = AnomalyTransformer(
            in_chunk_len=10, max_epochs=1, patience=1, pred_adjust=False)
        ...
        model = AnomalyTransformer(in_chunk_len=20, max_epochs=1, patience=1)
        ...
        model = AnomalyTransformer(
            in_chunk_len=20, max_epochs=1, patience=1, pred_adjust=False)
    ...
    def test_predict_score(self):
        ...
        """
        # case1 (index is DatetimeIndex and the feature is Univariate)
        model = AnomalyTransformer(in_chunk_len=10, max_epochs=1, patience=1)
        model.fit(self.tsdataset1, self.tsdataset1)
