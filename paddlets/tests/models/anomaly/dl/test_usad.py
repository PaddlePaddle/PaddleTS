...
from paddlets.models.anomaly import USAD
from paddlets.datasets import TimeSeries, TSDataset


class TestUSAD(TestCase):
    def setUp(self):
        """unittest function
        """
        np.random.seed(2022)
        ...
        self.tsdataset1 = TSDataset.load_from_dataframe(
            pd.concat(
        ...
        self.tsdataset2 = TSDataset.load_from_dataframe(
            pd.concat(
                [label, feature], axis=1), feature_cols=['a', 'b', 'c'])
        ...
        self.tsdataset3 = TSDataset.load_from_dataframe(
            pd.concat(
                [label, feature], axis=1),
            label_col='label',
            feature_cols='a')
        # There is no target in tsdataset
        self.tsdataset4 = TSDataset.load_from_dataframe(
            pd.concat(
                [label, feature], axis=1), feature_cols=['a', 'b', 'c'])
        # There is cate feature in tsdataset
        self.tsdataset5 = TSDataset.load_from_dataframe(
            pd.concat(
                [label, feature], axis=1),
            target_cols='label',
    ...
    def test_init(self):
        ...
        usad = USAD(in_chunk_len=16, **param1)
        ...
        with self.assertRaises(ValueError):
            usad = USAD(in_chunk_len=16, **param2)
        ...
        with self.assertRaises(ValueError):
            usad = USAD(in_chunk_len=16, **param3)
        ...
        with self.assertRaises(ValueError):
            usad = USAD(in_chunk_len=16, **param4)
        ...
        with self.assertRaises(ValueError):
            usad = USAD(in_chunk_len=16, **param5)
        ...
        with self.assertRaises(ValueError):
            usad = USAD(in_chunk_len=16, **param6)
            usad.fit(self.tsdataset1)
        ...
        usad = USAD(in_chunk_len=16, **param7)
        usad.fit(self.tsdataset1)
        self.assertIsInstance(usad._network._encoder._nn[1],
                              paddle.nn.BatchNorm1D)

    def test_init_dataloader(self):
        """unittest function for init dataloader
        """
        usad = USAD(in_chunk_len=16, max_epochs=1)
        #case 1 (valid dataset is empty)
        _, valid_dataloaders = usad._init_fit_dataloaders(self.tsdataset1)
        ...
        with self.assertRaises(ValueError):
            usad.fit(tsdataset, tsdataset)

    def test_init_metrics(self):
        """unittest function for init metrics
        """
        #case 1 (The User passed metrics)
        usad = USAD(in_chunk_len=16, max_epochs=1, eval_metrics=["mae"])
        _, metrics_names, _ = usad._init_metrics(["val"])
    ...
    def test_init_callbacks(self):
        ...
        """
        # case1 (patience = 0)
        usad = USAD(in_chunk_len=16, patience=0)
        usad._metrics, usad._metrics_names, _ = usad._init_metrics(["val"])
        with self.assertLogs("paddlets", level="WARNING") as captured:
            usad._init_callbacks()
        ...
        # case2 (patience > 0)
        usad = USAD(in_chunk_len=16, patience=1)
        usad._metrics, usad._metrics_names, _ = usad._init_metrics(["val"])
        _, callback_container = usad._init_callbacks()
        ...
        callback = Callback()
        usad = USAD(in_chunk_len=16, callbacks=[callback])
        usad._metrics, usad._metrics_names, _ = usad._init_metrics(["val"])
        _, callback_container = usad._init_callbacks()
    ...
    def test_fit(self):
        ...
        usad = USAD(
            ...
            max_epochs=20,
            patience=1)
        usad.fit(self.tsdataset1, self.tsdataset1)
        self.assertEqual(usad._stop_training, True)
    ...
    def test_predict(self):
        ...
        usad = USAD(
            ...
            patience=1)
        usad.fit(self.tsdataset1, self.tsdataset1)
        res = usad.predict(self.tsdataset1)
        ...
        usad = USAD(
            ...
            patience=1,
            ed_type='CNN')
        usad.fit(self.tsdataset2, self.tsdataset2)
        res = usad.predict(self.tsdataset2)
        self.assertIsInstance(res, TSDataset)
        self.assertIsInstance(res.get_target().data.index, pd.DatetimeIndex)
        self.assertEqual(res.get_target().data.shape, (185, 1))
        ...
        usad = USAD(
            ...
            max_epochs=1,
            patience=1)
        usad.fit(self.tsdataset3, self.tsdataset3)
        res = usad.predict(self.tsdataset3)
        self.assertIsInstance(res, TSDataset)
        ...
        usad = USAD(
            ...
            max_epochs=1,
            patience=1)
        usad.fit(self.tsdataset4, self.tsdataset4)
        res = usad.predict(self.tsdataset4)
        self.assertIsInstance(res, TSDataset)
        self.assertIsInstance(res.get_target().data.index, pd.RangeIndex)
        self.assertEqual(res.get_target().data.shape, (169, 1))
        self.assertEqual(res.get_target().data.columns[0], 'anomaly_label')
        ...
        usad = USAD(
            ...
            patience=1,
            embedding_size=16, )
        usad.fit(self.tsdataset5, self.tsdataset5)
        res = usad.predict(self.tsdataset5)
        self.assertIsInstance(res, TSDataset)
        self.assertIsInstance(res.get_target().data.index, pd.RangeIndex)
        self.assertEqual(res.get_target().data.shape, (185, 1))
        self.assertEqual(res.get_target().data.columns[0], 'label')
        ...
        usad = USAD(
            ...
            embedding_size=16, )
        usad.fit(self.tsdataset5, self.tsdataset5)
        res = usad.predict(self.tsdataset5, alpha=1.0, beta=0.0)
        self.assertIsInstance(res, TSDataset)
        self.assertIsInstance(res.get_target().data.index, pd.RangeIndex)
        self.assertEqual(res.get_target().data.shape, (185, 1))
        self.assertEqual(res.get_target().data.columns[0], 'label')
    ...
    def test_predict_score(self):
        ...
        usad = USAD(
            ...
            patience=1)
        usad.fit(self.tsdataset1, self.tsdataset1)
        res = usad.predict_score(self.tsdataset1)
        self.assertIsInstance(res, TSDataset)
        self.assertIsInstance(res.get_target().data.index, pd.DatetimeIndex)
        self.assertEqual(res.get_target().data.shape, (185, 1))
        ...
        usad = USAD(
            ...
            patience=1)
        usad.fit(self.tsdataset2, self.tsdataset2)
        res = usad.predict_score(self.tsdataset2)
        self.assertIsInstance(res, TSDataset)
        self.assertIsInstance(res.get_target().data.index, pd.DatetimeIndex)
        self.assertEqual(res.get_target().data.shape, (185, 1))
        ...
        usad = USAD(
            ...
            patience=1)
        usad.fit(self.tsdataset3, self.tsdataset3)
        res = usad.predict_score(self.tsdataset3)
        self.assertIsInstance(res, TSDataset)
        self.assertIsInstance(res.get_target().data.index, pd.RangeIndex)
        self.assertEqual(res.get_target().data.shape, (169, 1))
        self.assertEqual(res.get_target().data.columns[0], 'label_score')
        ...
        usad = USAD(
            ...
            ed_type='CNN')
        usad.fit(self.tsdataset4, self.tsdataset4)
        res = usad.predict_score(self.tsdataset4)
        self.assertIsInstance(res, TSDataset)
        self.assertIsInstance(res.get_target().data.index, pd.RangeIndex)
        self.assertEqual(res.get_target().data.shape, (169, 1))
        self.assertEqual(res.get_target().data.columns[0], 'anomaly_score')
        ...
        usad = USAD(
            ...
            embedding_size=16, )
        usad.fit(self.tsdataset5, self.tsdataset5)
        res = usad.predict_score(self.tsdataset5)
        self.assertIsInstance(res, TSDataset)
        self.assertIsInstance(res.get_target().data.index, pd.RangeIndex)
        self.assertEqual(res.get_target().data.shape, (185, 1))
        self.assertEqual(res.get_target().data.columns[0], 'label_score')
