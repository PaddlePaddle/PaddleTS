...
class TestUtils(unittest.TestCase):
    ...
    def __init__(self):
        ...
        self.tsdataset2 = TSDataset.load_from_dataframe(
            pd.concat(
                [label, feature], axis=1), feature_cols='a')
        # for forecasting
        self.tsdataset3 = TSDataset.load_from_dataframe(
            pd.concat(
                [target, feature], axis=1),
            target_cols='value',
            observed_cov_cols='a')
        ...
        self.tsdataset5 = TSDataset.load_from_dataframe(
            pd.concat(
                [label, feature], axis=1), feature_cols='a')
        # for forecasting
        self.tsdataset6 = TSDataset.load_from_dataframe(
            pd.concat(
                [target, feature], axis=1),
            target_cols='value',
            observed_cov_cols='a')
...
def utils_test_get_target_from_tsdataset(test_utils):
    ...
    with test_utils.assertLogs("paddlets", level="WARNING") as captured:
        ...
        test_utils.assertEqual(captured.records[0].getMessage(),
...
def utils_test_check_tsdataset(test_utils):
    ...
    test_utils.assertEqual(tsdataset.get_target().data['value'].dtype,
                           np.dtype('float32'))
    # case 2: test int
    tsdataset = test_utils.tsdataset1.copy()
    check_tsdataset(tsdataset)
    test_utils.assertEqual(tsdataset.get_target().data['label'].dtype,
                           np.dtype('int64'))
    # case 3: test nan
    tsdataset = test_utils.tsdataset3.copy()
