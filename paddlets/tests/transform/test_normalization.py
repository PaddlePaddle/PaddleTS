...
class test_MinMaxScaler(TestCase):
    ...
    def test_MinMax(self):
        ...
        input1 = TSDataset.load_from_dataframe(
            df=fake_input,
            known_cov_cols=['X', 'Y'],
        ...
        self.assertTrue(inverse1.get_known_cov().data.astype('int').equals(
            input1.get_known_cov().data.astype('int')))
    ...
    def test_with_sklearn(self):
        ...
        fake_input2['Y1'] = b1
        fake_input2['C'] = c
        input_ts = TSDataset.load_from_dataframe(
            df=fake_input2,
            known_cov_cols=['X', 'X1'],
        ...
        sk_fit_transform = scaler.fit(input_ts.to_numpy()).transform(
            input_ts.to_numpy())
