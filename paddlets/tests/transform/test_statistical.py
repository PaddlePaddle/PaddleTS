...
class TestStatsTransform(TestCase):
    ...
    def test_transform(self):
        ...
        fake_df = pd.DataFrame(
            np.arange(12.5, 32.5).reshape((5, 4)),
            columns=["WW", "XX", "YY", "ZZ"])
        ts = TSDataset.load_from_dataframe(
            df=fake_df, known_cov_cols=['WW', 'XX', 'YY', 'ZZ'])
        try:
            ob = StatsTransform(
                cols=[],
                start=0,
        ...
        try:
            ob = StatsTransform(
                cols=["WW"], start=0, end=2, statistics=['med', 'mean'])
        except Exception as e:
            succeed = False
            self.assertFalse(succeed)
        ...
        self.assertEqual(
            np.array(known_true_df).tolist()[1:],
            np.array(known[["WW_median", "WW_mean", "WW_min", "WW_max"]])
            .tolist()[1:])

        #��target�н���ʱ��ͳ��
        fake_input = pd.DataFrame(
            [[0.72, 0.29, 0.55, "2021-01-01 00:00:00"],
             [0.80, -0.84, -0.7, "2021-01-01 00:00:01"],
             [-1.2, 0.6, 2.1, "2021-01-01 00:00:02"],
             [0.6, 1.3, -0.4, "2021-01-01 00:00:03"]],
            columns=['value', 'cov1', 'cov2', 'time'])
