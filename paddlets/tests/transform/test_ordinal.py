...
class TestOrdinal(TestCase):
    ...
    def test_fit(self):
        ...
        ts = TSDataset.load_from_dataframe(
            df=ts_df,
            known_cov_cols=['Sex', 'Type'],
            observed_cov_cols=['Test'])
    ...
    def test_transform(self):
        ...
        observed_true_df = pd.DataFrame(
            [1.0, 1.0, 0.0, 0.0, 2.0], columns=['Test_encoder'])
        ...
        self.assertEqual(
            np.array(observed_true_df).tolist(), np.array(observed).tolist())
        ...
        for result in results:
            ...
            self.assertEqual(
                np.array(known_true_df).tolist(), np.array(known).tolist())
            self.assertEqual(
                np.array(observed_true_df).tolist(),
                np.array(observed).tolist())
