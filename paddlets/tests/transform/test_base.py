...
class TestUdBaseTransform(TestCase):
    ...
    def test__gen_input(self):
        ...
        #case1
        input = ud_transform._gen_input(self.dataset,
                                        self.dataset.target.columns[0])
        self.assertEqual(input.shape, (len(self.dataset.target.data), 1))
        #case2
        input = ud_transform._gen_input(self.dataset, [
            self.dataset.target.columns[0],
            *self.dataset.observed_cov.columns[:5]
        ])
    ...
    def test__check_output(self):
        ...
        #case2
        input_2 = ud_transform._gen_input(self.dataset, [
            self.dataset.target.columns[0],
            *self.dataset.observed_cov.columns[:5]
        ])
        ...
        #case5
        ud_transform = _UdMockTransform(
            MinMaxScaler(), drop_origin_columns=False)
        with self.assertRaises(ValueError):
            ud_transform._check_output(self.dataset, input_2, output_2)
        #case6
        output_3 = output[:-10, :]
        ...
        #case7
        ud_transform = _UdMockTransform(
            MinMaxScaler(), drop_origin_columns=True, out_col_names=['test1'])
        ud_transform._check_output(self.dataset, input, output)
        #case8
        ud_transform = _UdMockTransform(
            MinMaxScaler(),
        ...
        #case9
        ud_transform = _UdMockTransform(
            MinMaxScaler(), drop_origin_columns=False, out_col_types='target')
        ud_transform._check_output(self.dataset, input_2, output_2)
        ud_transform._check_output(self.dataset, input_2, output)
        #case10
    ...
    def test__infer_output_column_types(self):
        ...
        output = MinMaxScaler().fit_transform(input)
        self.assertEqual(
            ud_transform._infer_output_column_types(self.dataset, input),
            'target')
        ud_transform = _UdMockTransform(
            MinMaxScaler(),
        ...
        input_2 = ud_transform._gen_input(self.dataset, [
            self.dataset.target.columns[0],
            *self.dataset.observed_cov.columns[:2]
        ])
        output_2 = MinMaxScaler().fit_transform(input_2)
        self.assertEqual(
            ud_transform._infer_output_column_types(self.dataset, input_2),
            ['target', 'observed_cov', 'observed_cov'])
        #case2
        ud_transform = _UdMockTransform(
            MinMaxScaler(), drop_origin_columns=False)
        self.assertEqual(
    ...
    def test__get_output_column_names(self):
        ...
        #case1
        ud_transform = _UdMockTransform(
            MinMaxScaler(), drop_origin_columns=False, out_col_names=['name'])
        input = ud_transform._gen_input(self.dataset,
                                        self.dataset.target.columns[0])
        output = MinMaxScaler().fit_transform(input)
        self.assertEqual(
            ud_transform._get_output_column_names(input, output), ['name'])
        #case2
        ud_transform = _UdMockTransform(
            MinMaxScaler(), drop_origin_columns=True)
        input_2 = ud_transform._gen_input(self.dataset, [
    ...
    def test_fit(self):
        ...
        #case2
        ud_transform = _UdMockTransform(
            MinMaxScaler(), per_col_transform=True, drop_origin_columns=True)
        ud_transform.fit(self.dataset)
        self.assertEqual(
            len(ud_transform._ud_transformer_col_list),
    ...
    def test_transform(self):
        ...
        ud_transform.fit(self.dataset)
        dataset = ud_transform.transform(self.dataset)
        self.assertEqual(dataset.known_cov.data.shape,
                         (len(self.dataset.target), 3))
        #case5
        ud_transform = _UdMockTransform(
            MinMaxScaler(),
        ...
        ud_transform.fit(self.dataset)
        dataset = ud_transform.transform(self.dataset)
        self.assertEqual(dataset.observed_cov.data.shape,
                         (len(self.dataset.target), 13))
        #case6
        ud_transform = _UdMockTransform(
            MinMaxScaler(),
            in_col_names=['Visibility', 'DryBulbFarenheit'],
