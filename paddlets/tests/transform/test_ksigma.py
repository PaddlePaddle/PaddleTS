...
class TestKSigma(TestCase):
    ...
    def test_fit(self):
        ...
        ts = TSDataset.load_from_dataframe(
            df=ts_df,
            known_cov_cols=['Math', 'Eng'],
            observed_cov_cols=['Name'])

        #case1, a column
        ob = KSigma(['Math'])
        result = ob.fit(ts)
        expect_cols_stats_dict = {
            'Math': [58.75140453871301, 103.24859546128698, 81.0]
        }
        real_cols_stats_dict = result._cols_stats_dict
        for i in range(len(expect_cols_stats_dict['Math'])):
            self.assertAlmostEqual(expect_cols_stats_dict['Math'][i],
                                   real_cols_stats_dict['Math'][i])
        ...
        for col in ['Math', 'Eng']:
            for i in range(len(expect_cols_stats_dict[col])):
                self.assertAlmostEqual(expect_cols_stats_dict[col][i],
                                       real_cols_stats_dict[col][i])
    ...
    def test_transform(self):
        ...
        expect_known_df = pd.DataFrame(
            [90, 80, 70, 80, 85], columns=['Math'], dtype=float)
        real_known_df = result.get_known_cov().data
        self.assertEqual(
            np.array(expect_known_df['Math']).tolist(),
            np.array(real_known_df['Math']).tolist())

        #case2, two column
        ob = KSigma(['Math', 'Eng'])
        result = ob.fit(ts).transform(ts)
        expect_known_df = pd.DataFrame(
            [[90, 10], [80, 20], [70, 10], [80, 20], [85, 15]],
            columns=['Math', 'Eng'],
            dtype=float)
        real_known_df = result.get_known_cov().data
        for col in ['Math', 'Eng']:
            self.assertEqual(
                np.array(expect_known_df[col]).tolist(),
                np.array(real_known_df[col]).tolist())
        ...
        expect_known_df = pd.DataFrame(
            ...
            columns=['Math', 'Eng'],
            dtype=float)
        expect_observed_df = pd.DataFrame(
            ['e', 'e', 'd', 'd', 'f'], columns=['Name'])
        real_known_df = result.get_known_cov().data
        ...
        for col in ['Math', 'Eng']:
            self.assertEqual(
                ...
                np.array(real_known_df[col]).tolist())
        self.assertEqual(
            np.array(expect_observed_df['Name']).tolist(),
            np.array(real_observed_df['Name']).tolist())
        ...
        result = ob.fit(ts).transform(ts)
        expect_known_df = pd.DataFrame(
            [81, 81, 81, 81, 81], columns=['Math'], dtype=float)
        real_known_df = result.get_known_cov().data
        self.assertEqual(
            np.array(expect_known_df['Math']).tolist(),
            np.array(real_known_df['Math']).tolist())
