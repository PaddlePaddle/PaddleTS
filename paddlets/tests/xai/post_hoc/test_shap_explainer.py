...
class TestShapExplainer(TestCase):
    ...
    def test_explain(self):
        ...
        self.assertEqual(shap_value.shape, (out_chunk_len, 1, 16))
        ###########################################################################################################
        ...
        shap_value = se.explain(test_data, nsamples=20) #default the latest sample
        self.assertEqual(shap_value.shape, (out_chunk_len, 1, 12))
        ###########################################################################################################
        ...
        # Explainer
        se = ShapExplainer(pipe, train_data, background_sample_number=5, keep_index=True, use_paddleloader=False)
        shap_value = se.explain(test_data, nsamples=5) #default the latest sample
        self.assertEqual(shap_value.shape, (out_chunk_len, 1, 4))
        ###########################################################################################################
        ...
        self.assertEqual(shap_value.shape, (out_chunk_len, 1, 20))
        ###########################################################################################################
        ...
        # Explainer
        se = ShapExplainer(pipe._model, train_data, background_sample_number=5, keep_index=True, use_paddleloader=True)
        shap_value = se.explain(test_data, nsamples=40) #default the latest sample
        self.assertEqual(shap_value.shape, (out_chunk_len, 1, 32))
        ###########################################################################################################
        # Skip_chunk_len > 0
        df = self.data.to_dataframe()
        df['s'] = 0
    ...
    def test_different_model(self):
        ...
        # Pipeline
        pipeline_list = [(StandardScaler, {}), 
                 (TCNRegressor, {'in_chunk_len': in_chunk_len, 
                                'out_chunk_len': out_chunk_len, 
                                'skip_chunk_len': skip_chunk_len, 
                                'max_epochs': max_epochs, 
                                'patience': patience})
        ...
        # Pipeline
        pipeline_list = [(StandardScaler, {}), 
                 (NHiTSModel, {'in_chunk_len': in_chunk_len, 
                                'out_chunk_len': out_chunk_len, 
                                'skip_chunk_len': skip_chunk_len, 
                                'max_epochs': max_epochs, 
                                'patience': patience})
