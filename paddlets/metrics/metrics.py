...
class MSE(Metric):
    ...
    @ensure_2d
    def metric_fn(self, y_true: np.ndarray, y_score: np.ndarray) -> float:
...
class QuantileLoss(Metric):
    ...
    def __init__(
        ...
        raise_if_not(mode == "prob",
                     "QuantileLoss metric only support `prob` mode")
        if quantile_level is not None:
            logger.warning(
                f"The parameter `quantile_level` has been deprecated and will be removed in future update."
            )
    ...
    def __call__(
        ...
        """
        self._train = False
        return super(QuantileLoss, self).__call__(tsdataset_true,
                                                  tsdataset_pred)
    ...
    def metric_fn(
        ...
        quantiles = np.quantile(
            y_pred_sample, q_points, axis=-1, interpolation="nearest")
        errors = [y_true - quantiles[i] for i in range(len(q_points))]
        losses = [
            np.max(np.stack(
                [(q_points[i] - 1) * errors[i], q_points[i] * errors[i]],
                axis=-1),
                   axis=-1) for i in range(len(q_points))
        ]
        ...
        if self._train:
            ...
            return np.sum(losses_array, axis=-1).mean(
                axis=-1).mean()  # a scalar (shapeless)
        else:
...
class Precision(Metric):
    ...
    @ensure_2d
    def metric_fn(self, y_true: np.ndarray, y_score: np.ndarray,
                  **kwargs) -> float:
        """Precision_score.
