...
class Metric(ABC):
    ...
    def _build_metrics_data(
            ...
            tsdataset_pred: "TSDataset", ) -> Dict[str, Tuple[np.ndarray,
                                                              np.ndarray]]:
        ...
        raise_if(target_true is None or target_pred is None,
        ...
        raise_if_not((target_true.columns == target_pred.columns).all(),
                     "tsdataset true's and pred's columns are not the same!")
        target_pred = TimeSeries(
            target_pred.data.reindex(target_true.time_index), target_true.freq)
        ...
        for target in target_true.columns:
            res[target] = (target_true.data[target].to_numpy(),
                           target_pred.data[target].to_numpy())
        return res
    ...
    def _build_prob_metrics_data(
            ...
            data_type: str, ) -> Dict[str, Tuple[np.ndarray, np.ndarray]]:
        ...
        pred_target_set = set(
            [col.rsplit("@", 1)[0] for col in target_pred.columns])
        ...
        for target_name in target_true.columns:
            cur_pred_target_names = [
                x for x in target_pred_names
                if x.rsplit("@", 1)[0] == target_name
            ]
            target_pred_cur = target_pred[cur_pred_target_names]
            if data_type == "quantile":
                res[target_name] = (target_true[target_name].to_numpy(),
                                    target_pred_cur.to_numpy())
            else:  # data_type: "point"
                target_pred_cur_median = np.median(
                    target_pred_cur.to_numpy(), axis=-1)
                res[target_name] = (target_true[target_name].to_numpy(),
                                    target_pred_cur_median)
        return res
    ...
    def _build_anomaly_metrics_data(
        ...
        for column in target_true.columns:
            ...
            raise_if_not(
                operator.eq(true_value, [0, 1]) or
                operator.eq(true_value, [0]) or operator.eq(true_value, [1]),
                'In `anomaly` mode, the value in true label must be 0 or 1, please check your data!'
            )
            raise_if_not(
                operator.eq(pred_value, [0, 1]) or
                operator.eq(pred_value, [0]) or operator.eq(pred_value, [1]),
    ...
    def _reindex_data(self, target_true: TimeSeries,
                      target_pred: TimeSeries) -> List[TimeSeries]:
        """
        Reindex the true and pred data based on the time_index intersection.
        ...
        merge_index = pd.merge(
            target_true.time_index.to_frame(index=False),
            target_pred.time_index.to_frame(index=False))
        raise_if(
            len(merge_index) == 0,
        ...
        if len(merge_index) != len(target_true):
            logger.warning(
                "Tsdataset true's and pred's time_index do not match, the result will be calculated according to the intersection!"
            )
        ...
        target_true = TimeSeries(
            target_true.data.reindex(merge_index), target_true.freq)
        target_pred = TimeSeries(
            target_pred.data.reindex(merge_index), target_pred.freq)
    ...
    def __call__(
            self,
            tsdataset_true: "TSDataset",
            tsdataset_pred: "TSDataset", ) -> Dict[str, float]:
        """
        ...
        if self._mode == "normal":
            ...
            res_array = self._build_anomaly_metrics_data(tsdataset_true,
                                                         tsdataset_pred)
        res = {}
