...
class WeightingEnsembleBase(EnsembleBase):
    ...
    def predict(self, tsdataset: TSDataset) -> TSDataset:
        ...
        for name in target_names:
            meta = np.concatenate(
                [
                    np.array(prediction[name]).reshape(-1, 1)
...
class WeightingEnsembleForecaster(WeightingEnsembleBase, BaseModel):
    ...
    def __init__(self,
        ...
        raise_if_not(
            mode in FORECASTER_SUPPORT_MODES,
    ...
    def save(
            self,
            path: str,
            ensemble_file_name: str="paddlets-weighting-ensemble-forecaster-partial.pkl"
    ) -> None:
        """
...
class WeightingEnsembleAnomaly(WeightingEnsembleBase):
    ...
    def __init__(self,
                 ...
                 verbose: bool=False) -> None:
        raise_if_not(contamination < 0.5 and contamination >= 0,
                     "anomly_rate should in[0,0.5)")
        self._contamination = contamination

        raise_if_not(
            isinstance(in_chunk_len, int) and in_chunk_len > 0,
            "in_chunk_len should be a positive integer")
        self._in_chunk_len = in_chunk_len

        raise_if_not(
            mode in ANOMALY_SUPPORT_MODES,
    ...
    def fit(self,
        ...
        if self._mode != "voting":
            scores = self.predict_score(train_tsdataset).target.to_dataframe(
            ).to_numpy()
    ...
    def save(
            self,
            path: str,
            ensemble_file_name: str="paddlets-weighting-ensemble-anomly-partial.pkl"
    ) -> None:
        """
        Save the ensemble model to a directory.
