...
def main(args):
    ...
    if not os.path.exists(cfg_output_dir):
        ...
        if not os.path.isdir(cfg_output_dir):
            os.remove(cfg_output_dir)
            ...
            for f in os.listdir(cfg_output_dir):
                ...
                if f == 'best_model':
                    import shutil
                    shutil.rmtree(os.path.join(cfg_output_dir, f))
    ...
    if cfg.task == 'longforecast':
        ...
        if ts_val is not None:
            score = model.eval(ts_val)['mse']
        if cfg.model['name'] == 'PP-TS':
            model.save(cfg_output_dir + '/')
        else:
